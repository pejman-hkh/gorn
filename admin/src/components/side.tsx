...
function MenuSvg({ children, ...props }) {
    return <svg
...
function MenuText({ children, ...props }) {
    return <span {...props} class="ml-3" sidebar-toggle-item>{children}</span>
}
...
function MenuSearch({ }) {
    return <form action="#" method="GET" class="lg:hidden">
...
function MenuButtonText({ children }) {
    return <span class="flex-1 ml-3 text-left whitespace-nowrap" sidebar-toggle-item>{children}</span>
}

function MenuChilds({children, ...props}) {
    return  <ul id="dropdown-layouts" class="hidden py-2 space-y-2">{children}</ul>
}
